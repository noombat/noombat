...
async function deriveMasterKey(
  ...
  return crypto.subtle.importKey("raw", masterBits, "HKDF", false, ["deriveBits"]);
}

/** HKDF-Expand the master key with an info string. */
async function hkdfExpand(masterKey: CryptoKey, info: string): Promise<ArrayBuffer> {
  const enc = new TextEncoder();
...
export async function deriveBlobKey(
  ...
  return crypto.subtle.importKey("raw", blobBits, "AES-GCM", false, ["encrypt", "decrypt"]);
}
...
function setupRegisterForm(): void {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (password !== confirm) {
      showError(
        form,
        document.documentElement.dataset.passwordMismatch || "Passwords do not match.",
      );
      return;
    }
